...
BUILD_DIR="${BUILD_DIR:-/tmp/cyberpad-v03-final-s3-051}"
...
test -f "$OCD_HOME/share/openocd/scripts/board/esp32s3-builtin.cfg"
test -r "$CDC_GATE"
test -f "$BIN"
test -f "$BOOT"
test -f "$PART"
...
grep -Fq '"fqbn": "esp32:esp32:esp32s3:USBMode=default,CDCOnBoot=cdc,FlashSize=4M,PSRAM=disabled,UploadMode=cdc"' "$BUILD_DIR/build.options.json"
grep -Fq '/esp32/3.3.11' "$BUILD_DIR/build.options.json"

mapfile -t FLASH_ARGS < "$BUILD_DIR/flash_args"
EXPECTED_FLASH_ARGS=(
  '--flash-mode dio --flash-freq 80m --flash-size 4MB'
  '0x0 s3-dongle-validation.ino.bootloader.bin'
  '0x8000 s3-dongle-validation.ino.partitions.bin'
  '0xe000 boot_app0.bin'
  '0x10000 s3-dongle-validation.ino.bin'
)
...
for serial_file in /sys/bus/usb/devices/*/serial; do
  ...
  if [[ "$found_serial" == "$S3_SERIAL" && "$found_vid" == "303a" && "$found_pid" == "1001" ]]; then
...
[[ "$USB_IDENTITY_MATCHES" -eq 1 ]] || \
  die "expected exactly one S3 303a:1001 serial $S3_SERIAL in sysfs; found $USB_IDENTITY_MATCHES"
[[ "$USB_AUTHORIZED_MATCHES" -eq 1 && -n "$USB_DEVICE" ]] || \
  die "the exact S3 USB device is not uniquely present with authorized=1"
echo "==> exact S3 identity: $USB_DEVICE serial=$S3_SERIAL"
...
if bash "$CDC_GATE" prove "$CDC_WAIT_SECONDS" "$CDC_PROOF_SECONDS"; then
  echo "==> S3 release gate passed after the programmed reset"
  exit 0
fi

echo "==> S3 CDC proof did not appear; attempting one reset-only OpenOCD recovery"
# Recovery is intentionally reset-only. There is no second program_esp call and
# therefore no second flash opportunity in this invocation.
if ! sudo -n "$OCD_HOME/bin/openocd" -s "$OCD_HOME/share/openocd/scripts" \
  -f board/esp32s3-builtin.cfg \
  -c "adapter serial $S3_SERIAL" \
  -c "adapter speed 10000" \
  -c "init" \
  -c "reset run" \
  -c "shutdown"; then
  die "reset-only S3 recovery failed; firmware was not flashed a second time"
fi

bash "$CDC_GATE" prove "$CDC_WAIT_SECONDS" "$CDC_PROOF_SECONDS" || \
  die "S3 did not sustain exact CDC replies after the one allowed reset-only recovery"
echo "==> S3 release gate passed after one reset-only recovery"
