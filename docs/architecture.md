...
HK Operator is a configuration-driven input platform. Cybercat is the physical
ESP32-C6 controller. The HK Operator Mission Control Center (MCC) is the desktop
application that resolves Cybercat events against portable configuration.

## System overview

Three primary layers:

1. **Cybercat firmware** — switch matrix, presets, LEDs, BLE HID, custom GATT
...
- Discover and communicate with Cybercat through BlueZ
...
| --- | --- | --- |
| Preset index / LEDs | Cybercat firmware (runtime + NVS slots) | Hardware-local |
| Slot HID/macro payloads | Cybercat NVS (synced from MCC) | Written via GATT Slots characteristic |
...
- Physical input originates on Cybercat and crosses BLE to the host
...
| --- | --- | --- |
| Bluetooth disabled | Probe/MCC cannot find Cybercat | Enable adapter; `bluetoothctl power on` |
| Cybercat not paired | Status: not found | Pair as a Bluetooth keyboard first |
...
compiled into Cybercat firmware. Clone or sync profiles, pair Cybercat, launch MCC.

## Extension points

- Additional OS adapters (beyond clipboard / ydotool / open / shell)
- More physical Cybercat revisions (layout preserved)
...
- [Cybercat V1 hardware](./hardware-v1.md)
